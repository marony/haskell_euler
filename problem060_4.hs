...
process (p:ps) as = if length result1 >= count && length result2 > 0
                    then result2
                    else process ps (p : as)
--    where result1 = trace ("process' p = " ++ (show p) ++ ", as = " ++ (show as) ++ ", result1 = " ++ (show $ process' p as)) process' p as
    where result1 = process' p as
          comb = if length result1 >= count then combination result1 (count - 1) else []
--          comb = trace ("comb1 = " ++ (show $ combination result1 count)) combination result1 count
          result2 = check' p comb
          -- 自分と組み合わせたら素数になる素数をリスト化
          process' :: Int -> [Int] -> [Int]
--          process' p ass = trace ("process' p = " ++ (show p) ++ ", ass = " ++ (show ass)) p : filter (/= 0) (map f ass)
          process' p ass = filter f ass
          f :: Int -> Bool
          f x = (is_prime $ concatNumber p x) && (is_prime $ concatNumber x p)
          -- 全ての組み合わせが素数か調査
          check' :: Int -> [[Int]] -> [Int]
          check' _ [] = []
          check' p (xs:xss) = if all (\[x, y] -> (is_prime $ concatNumber x y) && (is_prime $ concatNumber y x)) comb
                              then (p : xs)
                              else check' p xss
              where comb = combination (p : xs) 2
